...
func canonicalTranscript(_ rows:[[String:String]])->Data { let sorted=rows.sorted{$0["id"]!<$1["id"]!}; let text="["+sorted.map{"{\"actual\":"+String(data:try! JSONSerialization.data(withJSONObject:$0["actual"]!),encoding:.utf8)!+",\"id\":"+String(data:try! JSONSerialization.data(withJSONObject:$0["id"]!),encoding:.utf8)!+"}"}.joined(separator:",")+"]"; return Data(text.utf8) }
