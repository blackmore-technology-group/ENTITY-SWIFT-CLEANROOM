...
**BTG-controlled Swift conformance baseline for ENTITY v3.3.**
...
This implementation exercises published ENTITY conformance campaigns in Swift, including the v3.2 adoption campaign and v3.3 Verifiable Reality campaign. CI verifies the sealed kit checksum before executing the language-native classifier.
...
`	ext
swift run -c release EntitySwiftCleanroom test`nswift run -c release AdoptionV32
`
...
A passing result shows that this BTG-controlled Swift implementation classifies the sealed public vectors consistently with the published campaign. It does **not** establish that a real-world claim is objectively true, and it does **not** establish independent external interoperability merely because another BTG-controlled language converges on the same result.
